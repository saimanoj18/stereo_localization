...
bool DepthMap::makeAndCheckEPL(const int x, const int y, const Frame* const ref, float* pepx, float* pepy, RunningStats* const stats)
{
    ...
    //std::cout<<"this to other: "<<ref->thisToOther_t<<std::endl;
...
void DepthMap::initializeFromStereo(Frame* left_frame, Frame* right_frame)
{
	activeKeyFramelock = left_frame->getActiveLock();
	activeKeyFrame = left_frame;
	activeKeyFrameImageData = activeKeyFrame->image(0);
	activeKeyFrameIsReactivated = false;

	const float* maxGradients = left_frame->maxGradients();
    right_frame->prepareForStereoWith(activeKeyFrame, right_frame->pose->thisToParent_raw, K, 0);
	...
	for(int y=1;y<height-1;y++)
	{
		for(int x=1;x<width-1;x++)
		{
			if(maxGradients[x+y*width] > MIN_ABS_GRAD_CREATE)
			{
                ...
                float epx, epy;
	            bool isGood = makeAndCheckEPL(x, y, right_frame, &epx, &epy, &runningStats);
                //std::cout<<epx<<", "<<epy<<std::endl;
	            if(isGood){
	                float result_idepth, result_var, result_eplLength;
	                float error = doLineStereo(
			                x,y,epx,epy,
			                0.0f, 1.0f, 1.0f/MIN_DEPTH,
			                right_frame, right_frame->image(0),
			                result_idepth, result_var, result_eplLength, &runningStats);
	                ...
	                currentDepthMap[x+y*width] = DepthMapPixelHypothesis(
			                ...
			                result_var,
			                VALIDITY_COUNTER_INITIAL_OBSERVE);
                    std::cout<<error<<": "<<result_idepth<<", "<<result_var<<std::endl;
                    //}
                }
			}
			else
	...
	RunningStats* stats)
{
	...
	if (firstX <= 0 || firstX >= width - 2
        ...
        //std::cout<<firstX<<", "<<lastX<<", "<<firstY<<", "<<lastY<<std::endl;		
        return -1;
	}
	...
	if(!(rescaleFactor > 0.7f && rescaleFactor < 1.4f))
	{
        ...
        //std::cout<<referenceFrame->K_otherToThis_R<<" and "<<rescaleFactor<<std::endl;
		return -1;
	}
